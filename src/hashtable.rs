...
impl<const L: usize, const T: usize> HashTable<L, T> {
    ...
    fn get<const B: usize, H: Tape>(
        ...
        let scaled_coords = coords.map(|[x, y]| [x as f32 * factor, y as f32 * factor]);

        let indices = scaled_coords.map(|[x, y]| {
            let [x, y] = [x.floor() as usize, y.floor() as usize];
            Self::CORNERS.map(|up| self.coord_to_index(res, [x + up[0], y + up[1]]))
        });

        let weights = scaled_coords.map(|[x, y]| {
            let [x, y] = [x.fract(), y.fract()];
            let x_w = [1. - x, x];
            let y_w = [1. - y, y];
            Self::CORNERS.map(|up| x_w[up[0]] * y_w[up[1]])
        });
        let weights: Tensor3D<B, 4, 2> = tensor(weights).broadcast();

        let selected: Tensor3D<B, 4, 2, H> = self.layers[layer].with_diff_tape().select(&indices);
